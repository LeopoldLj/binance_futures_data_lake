...
VERSION = "2026-02-14-ml-regime-week-v1"
...
CSV_OUT = Path(
    "/Users/lolo/PyCharmMiscProject/binance_futures_data_lake/data/research_debug/"
    "BTCUSDT/full_201910_202602/ml_regime_week_v1_summary.csv"
)
PRED_OUT = Path(
    "/Users/lolo/PyCharmMiscProject/binance_futures_data_lake/data/research_debug/"
    "BTCUSDT/full_201910_202602/ml_regime_week_v1_predictions.csv"
)
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Weekly regime ML v1 (supervised, 3-class).")
    ...
    p.add_argument("--horizon-weeks", type=int, default=3, help="Forward horizon in weeks for regime label.")
    p.add_argument("--trend-quantile", type=float, default=0.60, help="Train quantile on |future return| for trend threshold.")
    p.add_argument("--use-class-weight", action="store_true", help="Apply inverse-frequency class weighting in OvR training.")
...
def confusion_counts(y_true: np.ndarray, y_pred: np.ndarray) -> str:
    labels = [0, 1, 2]
    rows: List[str] = []
    for t in labels:
        for p in labels:
            n = int(((y_true == t) & (y_pred == p)).sum())
            rows.append(f"{LABEL_MAP[t]}->{LABEL_MAP[p]}:{n}")
    return ",".join(rows)


def build_weekly_frame(df: pd.DataFrame, ts_col: str, feat_cols: List[str]) -> pd.DataFrame:
    x = df.sort_values(ts_col).copy()
    x["_ts_naive"] = x[ts_col].dt.tz_localize(None)
    x["week_ts"] = x["_ts_naive"].dt.to_period("W-MON").dt.start_time

    ohlc = x.groupby("week_ts", sort=True).agg(
        open=("open", "first"),
        high=("high", "max"),
        low=("low", "min"),
        close=("close", "last"),
    )
    feat = x.groupby("week_ts", sort=True)[feat_cols].last()
    m = ohlc.join(feat, how="left").reset_index().rename(columns={"week_ts": "ts_w"})
    return m
...
def main() -> int:
    ...
    print(f"[run_ml_regime_week_v1] VERSION={VERSION}")
    print(f"[INFO] parquet_in={parquet_in}")

    schema = pd.read_parquet(parquet_in, engine="pyarrow")
    ts_col = auto_detect_ts_col(schema)
    w_cols = sorted([c for c in schema.columns if c.endswith("_w")])
    mn_cols = sorted([c for c in schema.columns if c.endswith("_mn")])
    feat_cols = sorted(set(w_cols + mn_cols))
    if not feat_cols:
        raise RuntimeError("No weekly/monthly context columns found (expected *_w / *_mn).")

    read_cols = [c for c in [ts_col, "open", "high", "low", "close"] + feat_cols if c in schema.columns]
    ...
    m = build_weekly_frame(df, ts_col=ts_col, feat_cols=feat_cols)
    m["ts_w"] = pd.to_datetime(m["ts_w"], utc=True, errors="coerce")
    m["ret_fwd"] = m["close"].shift(-int(args.horizon_weeks)) / m["close"] - 1.0
    m = m.dropna(subset=["ret_fwd"]).reset_index(drop=True)

    train_mask = m["ts_w"] < train_end
    val_mask = (m["ts_w"] >= train_end) & (m["ts_w"] < val_end)
    test_mask = m["ts_w"] >= val_end
    if train_mask.sum() < 24 or val_mask.sum() < 6 or test_mask.sum() < 6:
        raise RuntimeError(
            f"Not enough weekly samples. train={int(train_mask.sum())} val={int(val_mask.sum())} test={int(test_mask.sum())}"
        )
    ...
    class_weights = np.ones_like(y_train, dtype=float)
    if args.use_class_weight:
        vals, cnts = np.unique(y_train, return_counts=True)
        freq = {int(v): float(c) for v, c in zip(vals, cnts)}
        inv = {k: (len(y_train) / (len(vals) * max(v, 1.0))) for k, v in freq.items()}
        class_weights = np.array([inv.get(int(c), 1.0) for c in y_train], dtype=float)
        ...
        [
            {
                ...
                "confusion": confusion_counts(y_train, pred_train),
            },
            {
                "set": "VAL",
                ...
                "confusion": confusion_counts(y_val, pred_val),
            },
            {
                "set": "TEST",
                ...
                "confusion": confusion_counts(y_test, pred_test),
            },
        ]
    )

    pred_df = m[["ts_w", "close", "ret_fwd", "y"]].copy()
    ...
    print(f"[INFO] ts_col={ts_col} weekly_rows={len(m)} trend_threshold={thr:.6f} class_weight={bool(args.use_class_weight)}")
    print("\n=== ML WEEKLY REGIME V1 SUMMARY ===")
