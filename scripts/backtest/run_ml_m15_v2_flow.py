...
VERSION = "2026-02-14-ml-m15-v2-flow"
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="M15 ML v2 flow with temporal train/val/test and transaction costs.")
...
def to_m15_dataset(df: pd.DataFrame, ts_col: str, horizon: int) -> pd.DataFrame:
    ...
    agg = {
        ...
        "tradable_final": "last",
        "dir_ready": "last",
        "market_buys": "sum" if "market_buys" in d.columns else "mean",
        "market_sells": "sum" if "market_sells" in d.columns else "mean",
        "taker_buy_base": "sum" if "taker_buy_base" in d.columns else "mean",
        "volume": "sum" if "volume" in d.columns else "mean",
    }
...
def add_flow_features_m15(m15: pd.DataFrame) -> pd.DataFrame:
    out = m15.copy()

    # Robust buy/sell aggression reconstruction at M15.
    ...
    elif "taker_buy_base" in out.columns and "volume" in out.columns:
    ...
    total_aggr = (buys + sells).replace(0.0, np.nan)
    out["aggr_buy"] = buys.fillna(0.0)
    out["aggr_sell"] = sells.fillna(0.0)
    out["delta_aggr"] = out["aggr_buy"] - out["aggr_sell"]
    out["delta_aggr_norm"] = out["delta_aggr"] / total_aggr
    out["delta_aggr_norm"] = out["delta_aggr_norm"].replace([np.inf, -np.inf], np.nan).fillna(0.0)

    # CVD proxy from aggregated bar delta.
    out["cvd_proxy"] = out["delta_aggr"].cumsum()
    out["cvd_ret1"] = out["cvd_proxy"].diff()
    out["cvd_slope_3"] = out["cvd_proxy"] - out["cvd_proxy"].shift(3)
    out["cvd_slope_8"] = out["cvd_proxy"] - out["cvd_proxy"].shift(8)
    cvd_roll = out["cvd_ret1"].rolling(48, min_periods=20)
    out["cvd_ret1_z"] = (out["cvd_ret1"] - cvd_roll.mean()) / cvd_roll.std(ddof=0).replace(0.0, np.nan)

    # Divergence proxies price vs flow direction.
    out["price_slope_3"] = out["close"] - out["close"].shift(3)
    out["price_slope_8"] = out["close"] - out["close"].shift(8)
    out["div_cvd_price_3"] = out["cvd_slope_3"] * out["price_slope_3"]
    out["div_cvd_price_8"] = out["cvd_slope_8"] * out["price_slope_8"]
    out["div_cvd_price_3"] = np.sign(out["div_cvd_price_3"])
    out["div_cvd_price_8"] = np.sign(out["div_cvd_price_8"])

    flow_cols = [
        "cvd_ret1", "cvd_slope_3", "cvd_slope_8", "cvd_ret1_z",
        "price_slope_3", "price_slope_8", "div_cvd_price_3", "div_cvd_price_8",
    ]
    out[flow_cols] = out[flow_cols].replace([np.inf, -np.inf], np.nan).fillna(0.0)
    return out
...
def main() -> int:
    ...
    print(f"[run_ml_m15_v2_flow] VERSION={VERSION}")
    ...
    base_cols = [
        ...
        "market_buys", "market_sells", "taker_buy_base", "volume",
    ...
    feature_cols = [
        ...
        # flow/cvd pack
        "aggr_buy", "aggr_sell", "delta_aggr", "delta_aggr_norm", "cvd_proxy", "cvd_ret1", "cvd_slope_3",
        "cvd_slope_8", "cvd_ret1_z", "price_slope_3", "price_slope_8", "div_cvd_price_3", "div_cvd_price_8",
        # structure pack
        "vwap_dist", "vwap_dist_z", "hh_hl_state", "is_hh", "is_ll",
        "ema20_50_spread_m1", "ema50_200_spread_m1",
        "bb_width", "bb_pos", "kc_width", "squeeze_ratio",
