...
def build_daily_ohlcv(df: pd.DataFrame, ts_col: str, vol_col: str) -> pd.DataFrame:
    ...
    if "delta_norm" in x.columns:
        agg["delta_norm"] = "mean"
    m = x.resample("D", label="left", closed="left").agg(agg)
    m = m.dropna(subset=["open", "high", "low", "close"]).reset_index().rename(columns={ts_col: "bar_ts", vol_col: "volume"})
    return m


def add_day_features(m: pd.DataFrame, swing: int, bb_len: int, kc_len: int) -> pd.DataFrame:
    out = m.copy()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build Daily (D) MTF context features and project to base timeframe.")
    ...
    p.add_argument("--swing", type=int, default=6, help="Daily swing lookback for HH/LL.")
    p.add_argument("--bb-len", type=int, default=20)
    p.add_argument("--kc-len", type=int, default=20)
    p.add_argument("--shift-periods", type=int, default=1, help="Shift daily features by N periods (anti-lookahead).")
...
def main() -> int:
    ...
    print(f"[build_mtf_context_day_v1] VERSION={VERSION}")
    ...
    day = build_daily_ohlcv(df, ts_col=ts_col, vol_col=vol_col)
    day = add_day_features(day, swing=int(args.swing), bb_len=int(args.bb_len), kc_len=int(args.kc_len))
    ...
    # Anti-lookahead: project only completed daily bar context.
    shift_n = int(args.shift_periods)
    if shift_n > 0:
        day[feature_cols] = day[feature_cols].shift(shift_n)

    mn_cols = ["bar_ts"] + feature_cols
    mn_map = day[mn_cols].copy()
    rename_map = {c: f"{c}_d" for c in feature_cols}
    ...
    print("[INFO] daily_features=" + ", ".join([f"{c}_d" for c in feature_cols]))
