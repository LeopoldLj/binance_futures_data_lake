...
VERSION = "2026-02-14-mtf-h1-v1"
EPS = 1e-12


def auto_h1etect_ts_col(df: pd.DataFrame) -> str:
...
def normalize_ts_series(s: pd.Series) -> pd.Series:
    ...
    if np.issubdtype(s.dtype, np.integer):
        return pd.to_h1atetime(s, unit="ms", utc=True).dt.tz_convert(None)
    return pd.to_h1atetime(s, utc=True, errors="coerce").dt.tz_convert(None)
...
def build_h1_ohlcv(df: pd.DataFrame, ts_col: str, vol_col: str) -> pd.DataFrame:
    ...
    if "delta_norm" in x.columns:
        agg["delta_norm"] = "mean"
    m = x.resample("1h", label="left", closed="left").agg(agg)
    m = m.dropna(subset=["open", "high", "low", "close"]).reset_index().rename(columns={ts_col: "bar_ts", vol_col: "volume"})
    return m


def add_h1_features(m: pd.DataFrame, swing: int, bb_len: int, kc_len: int) -> pd.DataFrame:
    out = m.copy()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build H1 MTF context features and project to base timeframe.")
    ...
    p.add_argument("--swing", type=int, default=6, help="H1 swing lookback for HH/LL.")
    p.add_argument("--bb-len", type=int, default=20)
    p.add_argument("--kc-len", type=int, default=20)
    p.add_argument("--shift-periods", type=int, default=1, help="Shift H1 features by N periods (anti-lookahead).")
...
def main() -> int:
    ...
    print(f"[build_mtf_context_h1_v1] VERSION={VERSION}")
    ...
    ts_col = args.ts_col if args.ts_col else auto_h1etect_ts_col(schema)
    ...
    h1 = build_h1_ohlcv(df, ts_col=ts_col, vol_col=vol_col)
    h1 = add_h1_features(h1, swing=int(args.swing), bb_len=int(args.bb_len), kc_len=int(args.kc_len))
    ...
    # Anti-lookahead: project only completed h1 bar context.
    shift_n = int(args.shift_periods)
    if shift_n > 0:
        h1[feature_cols] = h1[feature_cols].shift(shift_n)

    mn_cols = ["bar_ts"] + feature_cols
    mn_map = h1[mn_cols].copy()
    rename_map = {c: f"{c}_h1" for c in feature_cols}
    ...
    print("[INFO] h1_features=" + ", ".join([f"{c}_h1" for c in feature_cols]))
