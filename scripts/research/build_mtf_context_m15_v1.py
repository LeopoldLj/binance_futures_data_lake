...
VERSION = "2026-02-14-mtf-m15-v1"
EPS = 1e-12


def auto_m15etect_ts_col(df: pd.DataFrame) -> str:
...
def normalize_ts_series(s: pd.Series) -> pd.Series:
    ...
    if np.issubdtype(s.dtype, np.integer):
        return pd.to_m15atetime(s, unit="ms", utc=True).dt.tz_convert(None)
    return pd.to_m15atetime(s, utc=True, errors="coerce").dt.tz_convert(None)
...
def build_m15_ohlcv(df: pd.DataFrame, ts_col: str, vol_col: str) -> pd.DataFrame:
    ...
    if "delta_norm" in x.columns:
        agg["delta_norm"] = "mean"
    m = x.resample("15min", label="left", closed="left").agg(agg)
    m = m.dropna(subset=["open", "high", "low", "close"]).reset_index().rename(columns={ts_col: "bar_ts", vol_col: "volume"})
    return m


def add_m15_features(m: pd.DataFrame, swing: int, bb_len: int, kc_len: int) -> pd.DataFrame:
    out = m.copy()
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Build M15 MTF context features and project to base timeframe.")
    ...
    p.add_argument("--swing", type=int, default=6, help="M15 swing lookback for HH/LL.")
    p.add_argument("--bb-len", type=int, default=20)
    p.add_argument("--kc-len", type=int, default=20)
    p.add_argument("--shift-periods", type=int, default=1, help="Shift M15 features by N periods (anti-lookahead).")
...
def main() -> int:
    ...
    print(f"[build_mtf_context_m15_v1] VERSION={VERSION}")
    ...
    ts_col = args.ts_col if args.ts_col else auto_m15etect_ts_col(schema)
    ...
    m15 = build_m15_ohlcv(df, ts_col=ts_col, vol_col=vol_col)
    m15 = add_m15_features(m15, swing=int(args.swing), bb_len=int(args.bb_len), kc_len=int(args.kc_len))
    ...
    # Anti-lookahead: project only completed m15 bar context.
    shift_n = int(args.shift_periods)
    if shift_n > 0:
        m15[feature_cols] = m15[feature_cols].shift(shift_n)

    mn_cols = ["bar_ts"] + feature_cols
    mn_map = m15[mn_cols].copy()
    rename_map = {c: f"{c}_m15" for c in feature_cols}
    ...
    new_cols_df = mapped.drop(columns=[ts_col]).copy()
    collisions = [c for c in new_cols_df.columns if c in full.columns]
    if collisions:
        rename_collisions = {c: f"{c}_ctx" for c in collisions}
        new_cols_df = new_cols_df.rename(columns=rename_collisions)
        print(f"[WARN] renamed_collisions={len(collisions)} (suffix=_ctx)")

    out = pd.concat([full, new_cols_df], axis=1)

    out_path.parent.mkdir(parents=True, exist_ok=True)
    out.to_parquet(out_path, index=False)

    print(f"[INFO] ts_col={ts_col} vol_col={vol_col} rows={len(out):,} cols={len(out.columns)}")
    print("[INFO] m15_features=" + ", ".join([f"{c}_m15" for c in feature_cols]))
